...
任何把外部内容写入 `raw/` 的动作（`wget` / `curl` / `git clone` / `WebFetch`）前，先读这两份文档：

- `ref/抓取工作流.md`：普适 SOP，含工具决策树、目录约定、留痕规范、图片边界、敏感信息检查、失败重试、X / 微信 / Medium / GitHub 等受限站点的处理边界
- `raw/下载说明.md`：当前批次状态，哪些已抓、哪些失败、下一步

红线：付费墙、登录态、私有仓库、需 API key 的接口、PII。绕付费墙抓取非公开内容禁止。具体边界见 `ref/抓取工作流.md` 第 1.2 节。
